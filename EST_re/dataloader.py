...
class NoExpDataset(Dataset):
    def __init__(self, classifier_tokenizer, device):
        self.tokenizer = classifier_tokenizer
        self.device = device
    ...
    def get_generator_dataset(self, args, classifier, retriever, generator_dataset, iters):
        ...
        for sen, retrieve_exp, similarity, label in zip(self.labeled_sentences, retrieve_exp_list, similarity_list, self.labels):
            ...
            tokens = pad_sequence([torch.tensor(x) for x in tokens], padding_value=self.tokenizer.pad_token_id, batch_first=True).to(args.device)
    ...
    def assign_pseudo_label(self, classifier, retriever, args, nums=10):
        ...
        class LocalDataset(Dataset):
            def __init__(self, samples):
                ...
                self.pos_list = all_pos

            def __len__(self):
                return len(self.samples)

            def __getitem__(self, index):
                return self.samples[index], self.pos_list[index]
        ...
        with torch.no_grad():
            ...
            for inputs, pos in tqdm.tqdm(test_iter):
