...
def get_args():
    ...
    parser.add_argument("--model_update_steps", default=1, type=int,
                        help="Pretrained config name or path if not the same as model_name")
    ...
    parser.add_argument('--num_iters', type=int, default=5,
                        help="Training epochs when initlizing.")
    parser.add_argument('--num_selftraing_iters', type=int, default=10,
                        help="Training epochs when initlizing.")
...
def main():
    ...
    noexpdataset = NoExpDataset(tokenizer, args.device)
    ...
    args.num_samples_per_iter = len(noexpdataset) // args.num_selftraing_iters

    valid_dataset = ValidDataset(tokenizer, args.device)
    valid_dataset.load_init(args.test_data, label_to_id=args.label_to_id)
    test_dataset = ValidDataset(tokenizer, args.device)
    ...
    self_training(generator, classifier, tokenizer, noexpdataset, generator_dataset, classifier_dataset, valid_dataset, test_dataset, retriever, args)
