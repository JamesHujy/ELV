...
def train_generator(generator, train_iter, optimizer, tokenizer, retriever, args, iters):
...
def train_classifier(classifier, train_iter, valid_dataset, optimizer, tokenizer, retriever, args, iters, total_epoch=-1):
    ...
    for epoch in range(total_epoch):
        for step, (inputs, labels) in enumerate(train_iter):
            ...
            loss = classifier(inputs, labels=labels)
            if torch.cuda.device_count() > 1:
                loss = loss.mean()
            if args.gradient_accumulation_steps > 1:
                loss = loss / args.gradient_accumulation_steps
            ...
            global_step += 1
        logger.info("Training Classifier Iters [%d] Epoch [%d] Step [%d]: %.2f", iters, epoch, global_step, logging_loss)
        logging_loss = 0.0
...
def valid_classifier(classifier, valid_dataset, tokenizer, retriever, args, iters, test=False):
    ...
    gd_rel_list = []
    entropy_list = []
    valid_classifier_dataset = valid_dataset.get_valid(retriever)
...
def train(generator, classifier, tokenizer, npexpdataset, generator_dataset, classifier_dataset, valid_dataset, test_dataset, retriever, args, selftrain_iter):
    ...
    train_generator(generator, generator_iter, generator_optimizer, tokenizer, retriever, args, -1)
    ...
    for iters in range(args.num_iters):
        ...
        train_classifier(classifier, classifier_iter, valid_dataset, classifier_optimizer, tokenizer, retriever, args, iters)

        precision, recall, f1_score = valid_classifier(classifier, valid_dataset, tokenizer, retriever, args, iters)
        precision_list_valid.append(precision)
        recall_list_valid.append(recall)
        f1_list_valid.append(f1_score)
        precision, recall, f1_score = valid_classifier(classifier, test_dataset, tokenizer, retriever, args, iters, test=True)
        ...
        generator_iter = DataLoader(generator_dataset, batch_size=args.batch_size, shuffle=True)
        train_generator(generator, generator_iter, generator_optimizer, tokenizer, retriever, args, iters)
        valid_generator(valid_dataset, tokenizer, args, iters)
        valid_generator(test_dataset, tokenizer, args, iters, test=True)
...
def self_training(generator, classifier, tokenizer, npexpdataset, generator_dataset, classifier_dataset, valid_dataset, test_dataset, retriever, args):
    classifier, classifier_optimizer = prepare_for_training(args, classifier)
    generator, generator_optimizer = prepare_for_training(args, generator)
    ...
    train_generator(generator, generator_iter, generator_optimizer, tokenizer, retriever, args, -1)
    classifier_iter = DataLoader(classifier_dataset, batch_size=args.batch_size, shuffle=True)
    train_classifier(classifier, classifier_iter, valid_dataset, classifier_optimizer, tokenizer, retriever, args, -1, total_epoch=15)
    precision, recall, f1_score = valid_classifier(classifier, test_dataset, tokenizer, retriever, args, -1, test=True)
    for i in range(args.num_selftrain_iters):
        ...
        if i > 0 and i % args.model_update_steps == 0:
            generator, classifier = get_model(args)
        train(generator, classifier, tokenizer, npexpdataset, generator_dataset, classifier_dataset, valid_dataset, test_dataset, retriever, args, i)
