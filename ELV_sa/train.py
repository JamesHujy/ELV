...
def train(generator, classifier, tokenizer, nolabeled_dataset, generator_dataset, classifier_dataset, valid_dataset, test_dataset, retriever, args):
    ...
    for iters in range(args.num_iters):
        classifier_dataset = nolabeled_dataset.get_classifier_dataset(args, generator, retriever, classifier_dataset, iters)
        ...
        generator_dataset = nolabeled_dataset.get_generator_dataset(args, classifier, retriever, generator_dataset, iters)
        generator_iter = DataLoader(generator_dataset, batch_size=args.batch_size, shuffle=True)
        ...
        if args.labeled_data_percent != 100:
            save_path = os.path.join(args.output_dir, "metrics_test")
        else:
            save_path = os.path.join(args.output_dir, "metrics_test_{}".format(args.labeled_data_percent))
